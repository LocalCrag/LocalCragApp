...
from models.enums.map_marker_type_enum import disabled_marker_types_crag, disabled_marker_types_sector, \
    disabled_marker_types_area, disabled_marker_types_topo_image, MapMarkerType
    ...
    def get(self):
        ...
        if 'area-id' in query_params:
            query = query.filter(
                or_(
                    ...
                    and_(
                        MapMarker.area_id == query_params['area-id'],
            ...
            query = query.filter(
                or_(
                    and_(
                        MapMarker.area.has(sector_id=query_params['sector-id']),
                        MapMarker.type == MapMarkerType.AREA
                    ),
                    and_(
                        MapMarker.sector_id == query_params['sector-id'],
                        MapMarker.type != MapMarkerType.SECTOR
                    )
                )
            )
            ...
            query = query.filter(
                or_(
                    and_(
                        MapMarker.sector.has(crag_id=query_params['crag-id']),
                        MapMarker.type == MapMarkerType.SECTOR
                    ),
                    and_(
                        MapMarker.crag_id == query_params['crag-id'],
                        MapMarker.type != MapMarkerType.CRAG
                    )
                )
            )
        else:
        ...
        if not get_show_secret():
            query = query.filter(
                and_(
                    or_(
                        MapMarker.crag.has(secret=False),
                        MapMarker.crag_id.is_(None)
                    ),
                    or_(
                        MapMarker.sector.has(secret=False),
                        MapMarker.sector_id.is_(None)
                    ),
                    or_(
                        MapMarker.area.has(secret=False),
                        MapMarker.area_id.is_(None)
                    )
                )
            )
