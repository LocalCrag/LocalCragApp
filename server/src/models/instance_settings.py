...
class InstanceSettings(db.Model):
    ...
    dark_logo_image_id = db.Column(UUID(), db.ForeignKey("files.id"), nullable=True)
    ...
    bg_image_id = db.Column(UUID(), db.ForeignKey("files.id"), nullable=True)
    bg_image = db.relationship("File", lazy="joined", foreign_keys=[bg_image_id])
    ...
    bar_chart_accent_color = db.Column(db.String(30), nullable=False, server_default="rgb(250, 204, 21)")
    dark_bar_chart_color = db.Column(db.String(30), nullable=False, server_default="rgb(248, 113, 113)")
    dark_bar_chart_accent_color = db.Column(db.String(30), nullable=False, server_default="rgb(253, 224, 71)")
    ...
    map_base_layers = db.Column(JSON, nullable=False, default=lambda: [], server_default="[]")
    map_overlays = db.Column(JSON, nullable=False, default=lambda: [], server_default="[]")
    ...
    # new release and notify everyone.
    release_notes_catalog_seeded = db.Column(db.Boolean, nullable=False, default=False, server_default="false")
