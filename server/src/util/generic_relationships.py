...
def get_object_secret(object_type, object_id):
    ...
    """
    if object_type == "Line":
        return db.session.get(Line, object_id).secret
    if object_type == "Area":
        return db.session.get(Area, object_id).secret
    if object_type == "Sector":
        return db.session.get(Sector, object_id).secret
    if object_type == "Crag":
        return db.session.get(Crag, object_id).secret
    return False
