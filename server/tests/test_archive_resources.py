...
def test_archive_topo_image(client, moderator_token, gym_mode):
    area_id = Area.get_id_by_slug("third-block-from-the-left")
...
def test_archive_second_topo_image_affects_only_associated_lines(client, moderator_token, gym_mode):
    ...
    In the test data, only "super-spreader" has a LinePath on order_index=1, while "stairs" does not.
    """
    area_id = Area.get_id_by_slug("third-block-from-the-left")
    ...
    # Only super-spreader should be archived, stairs untouched
    stairs = Line.find_by_slug("stairs")
    super_spreader = Line.find_by_slug("super-spreader")
    assert stairs.archived is False
    assert super_spreader.archived is True
...
def test_archive_area(client, moderator_token, gym_mode):
    ...
    archive_data = {
        ...
        "slug": "third-block-from-the-left",
        "value": True,
    }
...
def test_archive_sector(client, moderator_token, gym_mode):
    ...
    assert Line.query.filter_by(sector_slug="shade-valley", archived=False).count() == 0
    assert Line.query.filter_by(sector_slug="shade-valley", archived=True).count() > 0

    assert TopoImage.query.join(Area).filter(Area.sector_slug == "shade-valley", not TopoImage.archived).count() == 0
    assert TopoImage.query.join(Area).filter(Area.sector_slug == "shade-valley", TopoImage.archived).count() > 0
