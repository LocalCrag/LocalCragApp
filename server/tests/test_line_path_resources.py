...
def test_successful_order_line_paths(client, moderator_token):
    line_paths = (LinePath.query.join(LinePath.line)
                  .filter_by(area_id=Area.get_id_by_slug("dritter-block-von-links"))
                  .order_by(LinePath.order_index).all())
...
def test_successful_order_line_paths_for_line(client, moderator_token):
    line_paths = LinePath.query.filter_by(line_id=Line.get_id_by_slug("super-spreader")).order_by(LinePath.order_index).all()
