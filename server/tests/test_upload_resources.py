...
def test_successful_upload_small(client, clean_test_uploads, s3_mock, moderator_token):
    ...
    assert s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'], Key=res['filename']) is not None
    assert s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
                              Key='{}_xs.{}'.format(filename_parts[0], filename_parts[1])) is not None
    assert s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
                              Key='{}_s.{}'.format(filename_parts[0], filename_parts[1])) is not None
    with pytest.raises(Exception):
        s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
                           Key='{}_m.{}'.format(filename_parts[0], filename_parts[1]))
    with pytest.raises(Exception):
        s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
                           Key='{}_l.{}'.format(filename_parts[0], filename_parts[1]))
    with pytest.raises(Exception):
        s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
...
def test_successful_upload_medium(client, clean_test_uploads, s3_mock, moderator_token):
    ...
    assert s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
                              Key='{}_s.{}'.format(filename_parts[0], filename_parts[1])) is not None
    assert s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
                              Key='{}_m.{}'.format(filename_parts[0], filename_parts[1])) is not None
    with pytest.raises(Exception):
        s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
                           Key='{}_l.{}'.format(filename_parts[0], filename_parts[1]))
    with pytest.raises(Exception):
...
def test_successful_upload_large(client, clean_test_uploads, s3_mock, moderator_token):
    ...
    assert s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
                              Key='{}_m.{}'.format(filename_parts[0], filename_parts[1])) is not None
    assert s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
                              Key='{}_l.{}'.format(filename_parts[0], filename_parts[1])) is not None
    assert s3_mock.get_object(Bucket=current_app.config['SPACES_BUCKET'],
