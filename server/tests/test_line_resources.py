...
def test_successful_create_line(client, moderator_token):
    ...
    rv = client.post("/api/areas/third-block-from-the-left/lines", token=moderator_token, json=line_data)
...
def test_successful_get_lines_order_by_ascent_count_ascending(client):
    ...
    assert rv.status_code == 200
    res = rv.json["items"]
    assert [line["slug"] for line in res] == ["stairs", "super-spreader"]
...
def test_successful_get_lines_order_by_ascent_count_descending(client):
    ...
    assert rv.status_code == 200
    res = rv.json["items"]
    assert [line["slug"] for line in res] == ["super-spreader", "stairs"]
...
def test_successful_get_lines_order_by_time_created_ascending(client):
    stairs = Line.find_by_slug("stairs")
...
def test_get_lines_order_by_topo_position_respects_area_order_within_sector(client):
    ...
    first_area = Area.find_by_slug("third-block-from-the-left")
    ...
    rv = client.get("/api/lines?sector_slug=shade-valley&order_by=topo_position&order_direction=asc")
    assert rv.status_code == 200
    ...
    rv = client.get("/api/lines?sector_slug=shade-valley&order_by=topo_position&order_direction=desc")
    assert rv.status_code == 200
    assert [line["slug"] for line in rv.json["items"]] == [
        "super-spreader",
        "stairs",
...
def test_successful_edit_line(client, moderator_token):
    ...
    rv = client.put("/api/lines/stairs", token=moderator_token, json=line_data)
    assert rv.status_code == 200
    res = rv.json
    assert res["name"] == "It"
    assert res["slug"] == "it"
...
def test_successful_get_lines_for_line_editor(client):
    rv = client.get("/api/lines/for-line-editor/third-block-from-the-left")
    assert rv.status_code == 200
    res = rv.json
    assert len(res) == 2
...
def test_get_lines_required_bools_highball_filters_lines(client):
    ...
    line: Line = Line.find_by_slug("stairs")
    line.highball = True
    db.session.add(line)
    db.session.commit()
