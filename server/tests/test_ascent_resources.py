...
def test_successful_create_ascent(client, member_token):
    ascent_data = {
        ...
        "year": None,
        "gradeValue": 11,
        "line": str(Line.get_id_by_slug("stairs")),
        "date": "2024-04-13",
    }
    ...
    assert res["line"]["slug"] == "stairs"
    assert res["area"]["slug"] == "third-block-from-the-left"
    assert res["sector"]["slug"] == "shade-valley"
    assert res["crag"]["slug"] == "brione"

    # Check updated ascent counts
    rv = client.get("/api/lines/stairs")
...
def test_successful_get_ascents(client):
    ...
    assert res["items"][0]["area"]["slug"] == "third-block-from-the-left"
    assert res["items"][0]["sector"]["slug"] == "shade-valley"
...
def test_send_project_climbed_message(client, smtp_mock, moderator_token, user_token):
    ...
    stairs = str(Line.get_id_by_slug("stairs"))
...
def test_moderator_clear_ascent_fa_idempotent(client, moderator_token, admin_token):
    # Seed data only has FA ascents; create a non-FA tick (admin has super-spreader, not stairs).
    ascent_data = {
        "flash": False,
        "fa": False,
        "soft": False,
