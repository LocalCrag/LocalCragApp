...
export class AccountFormComponent {
  ...
  ngOnInit(): void {
    ...
    this.store.select(selectInstanceName).subscribe(instanceName => {
      this.title.setTitle(`${this.translocoService.translate(marker('accountFormTabTitle'))} - ${instanceName}`)
    });
  }
  ...
  public save() {
    if (this.accountForm.valid) {
      ...
      const user = new User();
      const emailChanged = this.currentUser.email !== this.accountForm.get('email').value;
      ...
      this.usersService.updateAccount(user).subscribe(updatedUser => {
        ...
        this.store.dispatch(toastNotification(NotificationIdentifier.ACCOUNT_SETTINGS_UPDATED));
