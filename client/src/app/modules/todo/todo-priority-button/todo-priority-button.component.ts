...
export class TodoPriorityButtonComponent implements OnInit {
  ...
  setPriority(priority: TodoPriority) {
    this.todosService.updateTodoPriority(this.todo, priority).subscribe(todo => {
      this.todo.priority = todo.priority
      this.store.dispatch(toastNotification(NotificationIdentifier.TODO_PRIORITY_UPDATED));
    });
  }
  ...
  ngOnInit() {
    this.items = [
      {
        label: this.translocoService.translate(marker('todos.priorityButton.highPriority')),
        icon: 'pi pi-angle-up',
        command: () => {
      ...
      {
        label: this.translocoService.translate(marker('todos.priorityButton.mediumPriority')),
      ...
      {
        label: this.translocoService.translate(marker('todos.priorityButton.lowPriority')),
