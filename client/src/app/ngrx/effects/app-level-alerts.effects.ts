...
export class AppLevelAlertsEffects {
   ...
   */
  onCheckShowCookieAlert = createEffect(() => this.actions$.pipe(
    ofType(checkShowCookieAlert),
    tap(() => {
      if (JSON.parse(localStorage.getItem('cookiesAccepted') as string) !== true) {
   ...
   */
  onCookiesAccepted = createEffect(() => this.actions$.pipe(
    ofType(cookiesAccepted),
    tap(() => {
      localStorage.setItem('cookiesAccepted', JSON.stringify(true));
      this.store.dispatch(toastNotification( NotificationIdentifier.COOKIES_ALLOWED));
    })
  ), {dispatch: false});
