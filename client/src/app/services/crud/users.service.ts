...
export class UsersService {
  ...
  public getUsers(): Observable<User[]> {
    return this.http.get(this.api.users.getList()).pipe(map((userListJson: any) => userListJson.map(User.deserialize)))
  }


  public getUser(slug: string): Observable<User> {
    return this.http.get(this.api.users.getDetail(slug)).pipe(map(User.deserialize))
  }
  ...
  public getUserGrades(userSlug: string): Observable<Grade[]> {
    return this.http.get(this.api.users.getGrades(userSlug)).pipe(map((gradeListJson: any) => gradeListJson.map(deserializeGrade)));
  }
